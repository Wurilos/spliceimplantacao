...
import { Plus, Save, Trash2, Edit, Calendar, Image, TrendingUp, Settings2 } from 'lucide-react';
import { supabase } from '@/integrations/supabase/client';
import { ImageThumbnail } from '@/components/ImageThumbnail';
import { useToast } from '@/hooks/use-toast';
import {
  useOperacionalItens,
  useCreateOperacionalItem,
  useUpdateOperacionalItem,
  useDeleteOperacionalItem,
  OperacionalItem,
} from '@/hooks/useOperacional';
import { useOperacionalCategoria, OperacionalCategoriaItem } from '@/hooks/useOperacionalCategoria';
...
export function OperacionalTab({
  ...
  const { data: itens } = useOperacionalItens(equipamentoId);
  const { data: categoriaItens, isLoading: isLoadingCategorias } = useOperacionalCategoria();
  const { data: previsoes } = useEquipamentoPrevisoes(equipamentoId);
  const bulkUpsertPrevisoes = useBulkUpsertEquipamentoPrevisoes();
  const createItem = useCreateOperacionalItem();
  const updateItem = useUpdateOperacionalItem();
  const deleteItem = useDeleteOperacionalItem();

  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingItem, setEditingItem] = useState<OperacionalItem | null>(null);
  ...
  // Calculate executed totals per category item
  const executadoPorCategoria = useMemo(() => {
  ...
  const openDialog = (item?: OperacionalItem) => {
    if (item) {
      setEditingItem(item);
      setItemForm({
      ...
      <Card className="shadow-soft border-l-4 border-l-warning">
        <CardContent className="py-12">
          <div className="text-center text-muted-foreground">
            ...
            <p className="font-medium mb-2">Nenhuma categoria operacional cadastrada</p>
            <p className="text-sm">
              Cadastre uma categoria com nome "Operacional" e adicione itens em{' '}
    ...
    <div className="space-y-6">
      ...
      <Card className="shadow-soft border-l-4 border-l-accent">
        <CardHeader className="pb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <CardTitle className="text-lg">Previsão Operacional</CardTitle>
              <CardDescription>Quantidade prevista de itens operacionais</CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent>
        ...
        <CardContent>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {categoriaItens.map((item) => {
              ...
              const executado = executadoPorCategoria[item.id] || 0;
              const percentual = previsto > 0 ? Math.round((executado / previsto) * 100) : 0;
              
              return (
                <div key={item.id} className="p-3 rounded-lg border bg-card">
                  <div className="text-sm font-medium text-muted-foreground">{item.nome}</div>
                  <div className="flex items-baseline gap-2 mt-1">
                    <span className="text-2xl font-bold">{executado}</span>
                    <span className="text-sm text-muted-foreground">/ {previsto}</span>
                  </div>
                  <div className="w-full h-1.5 bg-muted rounded-full mt-2 overflow-hidden">
                    <div 
                      className={`h-full transition-all ${percentual >= 100 ? 'bg-success' : 'bg-accent'}`}
        ...
        <CardHeader className="flex flex-row items-center justify-between pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>Registro de operações realizadas</CardDescription>
            </div>
          </div>
          {canEdit && (
        ...
        <CardContent>
          {itens?.length === 0 ? (
            <div className="text-center py-12 text-muted-foreground border-2 border-dashed rounded-lg">
              <Settings2 className="h-12 w-12 mx-auto mb-3 opacity-20" />
              <p>Nenhum item operacional registrado</p>
            </div>
          ) : (
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>{editingItem ? 'Editar Item' : 'Novo Item Operacional'}</DialogTitle>
            <DialogDescription>
