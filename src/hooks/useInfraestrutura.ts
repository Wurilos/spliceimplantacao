...
export function useInfraestruturaItens(equipamentoId: string | undefined) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return (data || []) as unknown as InfraestruturaItem[];
    },
...
export function useCreateInfraestruturaItem() {
  ...
  return useMutation({
    mutationFn: async (item: Omit<InfraestruturaItem, 'id' | 'created_at' | 'updated_at' | 'categoria_itens'>) => {
      const { data, error } = await supabase
        .from('infraestrutura_itens')
        .insert(item as any)
        .select()
...
export function useUpdateInfraestruturaItem() {
  ...
  return useMutation({
    mutationFn: async ({ id, ...item }: Partial<Omit<InfraestruturaItem, 'categoria_itens'>> & { id: string }) => {
      const { data, error } = await supabase
        .from('infraestrutura_itens')
        .update(item as any)
        .eq('id', id)
